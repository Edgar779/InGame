...
export class FileController {
  ...
  async uploadImage(
    ...
    const uploadedFile = await this.imagesService.create(file, includeThumbnail);
    return uploadedFile;
  }
  ...
  async uploadImages(
    ...
    const uploadedFiles = await this.imagesService.createMany(files, includeThumbnail);
  ...
  @Delete(':id')
  async delete(@Param('id', ParseObjectIdPipe) id: string, @Body('user') uploader: string): Promise<any> {
